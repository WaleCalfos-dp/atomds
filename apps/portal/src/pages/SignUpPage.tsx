  ...
  return (
    <div className="min-h-screen grid place-items-center p-6">
      <form onSubmit={onSubmit} className="w-full max-w-sm space-y-4 bg-white p-6 rounded-md border border-gray-200 shadow-sm">
        ...
        <p className="text-sm text-gray-600">Email confirmation must be disabled in your Supabase project's Auth settings for instant sign-in.</p>
        ...
        <label className="block text-sm">
          ...
          <input type="password" required minLength={6} value={password} onChange={(e) => setPassword(e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-md" />
        </label>
        ...
        <p className="text-sm text-gray-600">
          Already have one? <Link to="/sign-in" className="text-gray-900 underline">Sign in</Link>
        </p>
      </form>
    </div>
  )
