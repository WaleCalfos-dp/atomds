...
import { type Brand, BRANDS } from '@atom-core/tokens/tokens';
  ...
  return (
    <header className="fixed top-0 left-60 right-0 h-14 bg-white border-b border-slate-200 flex items-center justify-between px-6 z-10">
      ...
      <div className="flex items-center gap-0.5">
        ...
        {studioBrand ? (
          <button
            onClick={() => setBrand('studio')}
            title={`${studioBrand.name} (${t.studioBrandLabel})`}
            className={[
              'flex items-center gap-1.5 px-2.5 py-1 rounded-md text-xs font-medium transition-all duration-150',
              brand === 'studio'
                ? 'bg-slate-900 text-white shadow-sm'
          ...
          >
            {studioBrand.logo ? (
              ...
              <span
                className="w-2 h-2 rounded-full flex-shrink-0"
                style={{ backgroundColor: studioBrand.seeds.primary }}
              />
            )}
            {studioBrand.name || t.studioBrandLabel}
          </button>
        ) : (
          <button
